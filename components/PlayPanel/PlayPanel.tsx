...
const PlayPanel = (props: IMyProps): ReactElement => {
                ...
                for (let i = 0; i < resultobj.length; i++) {
                    ...
                    if (trimText.slice(-1) !== '.') {
                        ...
                        if (phrase === '') {
                            ...
                            if (
                                resultobj[i + 1] &&
                                resultobj[i + 1].text[0] === resultobj[i + 1].text[0].toUpperCase()
                            ) {
                                phrase += ' ' + trimText
                                duration += resultobj[i].duration
                                mergeObj.push({
    ...
    useEffect(() => {
        ...
        if (caption) {
            ...
            ref.current?.seekTo(caption[currentCaptionIndex].start, 'seconds')
            console.log(`seekTo: ${caption[currentCaptionIndex].start}`)
            ...
            <div className="fixed top-0 left-0 right-0 bottom-0 grid content-center justify-center">
                <div style={{ width: '640px' }}>
                    ...
                    <div id="controlBar" className="flex justify-evenly my-3">
                        ...
                        <div>
                            {testMode ? (
                                <Grid onClick={switchMode} size={24} />
                            ) : (
                                <Type onClick={switchMode} size={24} />
                            )}
                        </div>
                        <div>
                            {
                    ...
                    <div id="typingArea">
                        {testMode ? (
                            <TypingPanel
                                captionText={caption[currentCaptionIndex].text}
                                replayFunc={switchRepeat}
                                captionIndex={currentCaptionIndex}
                            />
                        ) : (
                            <TestPanel captionText={caption[currentCaptionIndex].text} />
                        )}
                    </div>
                </div>
            </div>
